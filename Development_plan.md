...
* `primary` 真實抓取已在 Docker 版 n8n 內驗證成功
* `Execute Command` 節點已可用
* 最小流程已跑到 `Manual Trigger -> Execute Command -> Code`

明天優先做：

1. 在 n8n 補上 Telegram 節點，先完成 **單圖貼文送出**
2. 把流程補成依 `delivery_type` 分流：
   * `message` -> Send Message
   * `photo` -> Send Photo
   * `media_group` -> Send Media Group
3. 驗證 caption / follow-up message 行為在 n8n 內正常
4. 將最終可用的 n8n workflow 回寫到 repo 的 sample workflow
5. 視情況開始補 **backup scraper** 接入
...
如果之後要清掉當前記憶、開新會話，建議先重新讀這 3 個檔案：
...
* `Development_plan.md` 保留原始目標與後續階段規劃
* `Development_history.md` 記錄目前已完成的實作與驗證結果
* `README.md` 反映目前 CLI、schema、n8n 與 Telegram 輸出介面的實際狀態

建議的新會話起手式：

```text
專注 /home/roger/WorkSpace/youtube_post_repeater
先讀 Development_plan.md、Development_history.md、README.md
再檢查目前程式碼狀態，接著繼續下一步
```
